...
// One-time bootstrap: promotes an existing account to superadmin using a
// server-side secret (not a JWT), since there's no admin yet to do it via
// the normal admin routes. Meant to be run once by you, then effectively
// unused - every college admin after this is created via POST /api/admin/users.
app.post('/api/admin/make-admin', withDB(async (req, res) => {
    try {
        ...
        res.json({ success: true, message: `${email} is now the super admin` });
    } catch (err) {
        console.error(err);
        res.status(500).json({ success: false, message: 'Server error' });
    }
